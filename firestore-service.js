...
/**
 * (NOVO) Marca um alvo já arquivado como "respondido", sem movê-lo de coleção.
 * @param {string} uid - ID do usuário.
...
/**
 * (NOVO) Restaura um alvo da coleção de arquivados para a de ativos.
 * @param {string} uid - ID do usuário.
 * @param {object} target - O objeto completo do alvo a ser restaurado.
 */
export async function restoreArchivedTarget(uid, target) {
    const batch = writeBatch(db);
    
    // Referências para o documento antigo (em arquivados) e o novo (em ativos)
    const oldRef = doc(db, "users", uid, "archivedTargets", target.id);
    const newRef = doc(db, "users", uid, "prayerTargets", target.id);

    // Prepara os dados para serem restaurados, limpando os campos de arquivamento
    ...
    delete dataToRestore.id; // Remove o ID temporário do front-end antes de salvar

    // Adiciona as operações ao batch: criar o novo e deletar o antigo
