    ...
    return rawData.map(targetData => {
        ...
        if (Array.isArray(rehydrated.observations)) {
            rehydrated.observations = rehydrated.observations.map(obs => {
                // A observação agora pode ser um Timestamp ou um objeto com um Timestamp
                const obsDate = obs.date || obs; // Compatibilidade com estrutura antiga
                if (obsDate instanceof Timestamp) {
                    return { ...obs, date: obsDate.toDate() };
                }
                return obs;
            });
        }
...
/**
 * (NOVO/REFATORADO) Adiciona uma observação a um alvo, utilizando o helper interno.
 * @param {string} uid ID do usuário.
...
/**
 * (NOVO) Busca TODOS os alvos de um usuário (ativos e arquivados) para o relatório.
