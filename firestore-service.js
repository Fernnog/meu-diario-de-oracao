// =================== INÍCIO DO ARQUIVO firestore-service.js ===================
...
/**
 * Obtém a data atual no formato string YYYY-MM-DD, usando UTC para consistência.
 * @param {Date} date - O objeto de data a ser formatado.
 * @returns {string} - A data formatada.
 */
function getISODateStringUTC(date) {
    if (!date) return null;
...
// CORREÇÃO: Adicionada a palavra-chave 'export' que estava faltando.
...
export async function loadDailyTargets(uid, allActiveTargets) {
    const todayStr = getISODateStringUTC(new Date());
...
export async function recordUserInteraction(uid, currentPerseveranceData, currentWeeklyData) {
    ...
    const todayStr = getISODateStringUTC(today);
    ...
    const lastDateStr = lastInteractionDate ? getISODateStringUTC(lastInteractionDate) : null;
    
    if (lastDateStr !== todayStr) {
        const yesterday = new Date();
        yesterday.setUTCDate(today.getUTCDate() - 1);
        const yesterdayStr = getISODateStringUTC(yesterday);
...
// =================== FIM DO ARQUIVO firestore-service.js ===================
