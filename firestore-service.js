...
export async function loadDailyTargets(uid, allActiveTargets) {
    ...
    try {
        ...
        const completedTargets = dailyData.targets.filter(t => t.completed);

        const pending = allActiveTargets.filter(t => targetIds.includes(t.id) && !completedTargets.some(ct => ct.targetId === t.id));
        const completedFromActive = allActiveTargets.filter(t => completedTargets.some(ct => ct.targetId === t.id));

        // Para sub-alvos orados
        const completedSubTargets = completedTargets.filter(t => t.isSubTarget);
