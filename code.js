...
function renderObservations(observations) {
    ...
    if (observations.length > 0) {
        ...
        // Removed <h4>Observações:</h4> from here
        return `
            <div class="observations-container">
                <button class="observations-toggle" data-target-id="${uniqueId}" onclick="toggleObservations('${uniqueId}')">Observações</button>
                <div id="observations-${uniqueId}" class="observations-list-collapsible" style="display: none; margin-top: 5px; padding-left: 10px;">
...
window.toggleObservations = function(targetId) { // Make toggleObservations globally accessible
    ...
    const button = document.querySelector(`.observations-toggle[data-target-id="${targetId}"]`);
    observationsDiv.style.display = observationsDiv.style.display === 'none' ? 'block' : 'none';
    button.classList.toggle('active', observationsDiv.style.display === 'block'); // Toggle 'active' class
}
...
function refreshDailyTargets() {
   ...
   // Se todos os alvos foram exibidos, reseta o histórico
    ...
    selectedTargets.forEach((target, index) => {
        ...
        dailyDiv.innerHTML += renderObservations(target.observations); // Use renderObservations here!
        dailyTargets.appendChild(dailyDiv);
         dailyDiv.querySelector('.observations-list-collapsible').style.display = 'none'; // Ensure daily targets are collapsed
