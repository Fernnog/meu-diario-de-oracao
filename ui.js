...
export function generateViewHTML(targets, pageTitle, selectedCategories = []) {
    ...
    // A LÓGICA CORRIGIDA E SIMPLIFICADA:
    // Itera APENAS sobre os alvos já filtrados que foram passados para a função.
    for (const target of targets) {
        const category = target.category || 'Sem Categoria';
        
        // Se a categoria ainda não existe no nosso objeto de grupos, crie-a.
        if (!groupedTargets[category]) {
            groupedTargets[category] = [];
        }
        // Adiciona o alvo ao seu grupo correspondente.
        groupedTargets[category].push(target);
    }

    // Construir o conteúdo HTML a partir dos grupos
    let bodyContent = '';
    const sortedCategories = Object.keys(groupedTargets).sort();

    for (const category of sortedCategories) {
        // Apenas renderiza a seção se houver alvos nela
    ...
    if (bodyContent === '') {
        bodyContent = '<p>Nenhum alvo encontrado para as categorias selecionadas.</p>';
    }

    // Retornar o template completo com o botão de imprimir
    return `
