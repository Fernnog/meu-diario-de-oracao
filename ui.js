...
// ARQUITETURA REVISADA: Inclui a lógica para renderizar observações como sub-alvos.
...
/**
 * (RECONSTRUÍDO - PRIORIDADES 1 & 2)
 * Gera o HTML para a lista de observações de um alvo, diferenciando entre
 * observações normais, sub-alvos e as observações aninhadas de um sub-alvo.
 * @param {Array<object>} observations - O array de observações do alvo principal.
...
function createObservationsHTML(observations, parentTargetId) {
    ...
    // Ordena as observações da mais antiga para a mais recente para uma leitura cronológica
    ...
    sorted.forEach((obs, index) => {
        ...
        if (obs.isSubTarget) {
            // ----- RENDERIZA COMO UM SUB-ALVO (Prioridade 1) -----
            const isResolved = obs.subTargetStatus === 'resolved';
            
            // Botões de ação para sub-alvos
            const actions = !isResolved ? `
                ...
                <button class="btn-small" data-action="edit-sub-target-title" data-id="${parentTargetId}" data-obs-index="${index}">Editar Título</button>
            ` : `<span class="resolved-tag">Respondido</span>`;

            // INÍCIO - Lógica para renderizar sub-observações (Prioridade 2)
            ...
            if (Array.isArray(obs.subObservations) && obs.subObservations.length > 0) {
                ...
                subObservationsHTML = `
                    <div class="sub-observation-container">
                        <h4>Diário do Sub-Alvo:</h4>
                        <div class="sub-observation-list">
                            ${sortedSubObs.map(subObs => `<p><strong>${formatDateForDisplay(subObs.date)}:</strong> ${subObs.text.replace(/</g, "&lt;").replace(/>/g, "&gt;")}</p>`).join('')}
                        </div>
                    </div>`;
            }
            // FIM - Lógica para renderizar sub-observações
                ...
                <div class="observation-item sub-target ${isResolved ? 'resolved' : ''}">
                    <div class="sub-target-header">
                        ...
                        <div class="observation-actions">
                           ${actions}
                           <button class="btn-small" data-action="demote-sub-target" data-id="${parentTargetId}" data-obs-index="${index}">Reverter</button>
                        </div>
                    </div>
...
function createTargetHTML(target, config = {}, dailyTargetsData = {}) {
    // (NOVO) Ícone indicador de sub-alvos
    ...
    // (MODIFICADO) Passa o ID do alvo para a função de observações.
    ...
    return `
        ...
        <style>
            ...
            .view-separator { border: 0; border-top: 1px solid #ccc; margin: 20px 0; }
             /* Estilos para a visualização de subalvos */
            .observation-item.sub-target { background-color: #f0f8ff; border-left: 3px solid #add8e6; padding: 10px; margin-top: 10px; }
            .sub-target-title { font-weight: bold; color: #00008b; }
            .sub-observation-container { margin-top: 10px; padding-left: 15px; border-left: 2px dotted #ccc; }
        </style>
