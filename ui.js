...
// ARQUITETURA REVISADA: Inclui a lógica para renderizar observações, sub-alvos, as observações dos sub-alvos e o novo formulário de edição de prazo.
...
function createTargetHTML(target, config = {}, dailyTargetsData = {}) {
    ...
    if (config.showActions) {
        ...
        // MELHORIA APLICADA: O botão "Editar Prazo" agora é exibido se a configuração permitir,
        // não dependendo mais se o alvo já tem um prazo ou não.
...
/**
 * (NOVA FUNÇÃO) Mostra ou esconde o formulário inline para editar o prazo de um alvo.
 * @param {string} targetId - O ID do alvo.
 * @param {Date | null} currentDeadline - A data de prazo atual, ou null.
 */
export function toggleEditDeadlineForm(targetId, currentDeadline) {
    ...
    // Garante que outros formulários inline estejam fechados para uma UI limpa
    ...
    if (isVisible) {
        ...
        formDiv.innerHTML = `
            <label for="deadlineInput-${targetId}">Definir Prazo:</label>
