...
/**
 * [CORRIGIDO] Atualiza o quadro de interação semanal.
 * A lógica foi refatorada para usar estritamente UTC ao calcular os dias da semana,
 * evitando bugs de fuso horário que causavam a marcação incorreta de dias anteriores.
 * A borda do "dia atual" continua usando o fuso horário local para uma UX intuitiva.
 * @param {object} data - Objeto contendo os dados de interação.
 */
export function updateWeeklyChart(data) {
    ...
    // A borda verde deve refletir o "hoje" na percepção do usuário.
    const localDayOfWeek = now.getDay(); // 0=Dom, 1=Seg, ... (baseado no fuso do navegador)

    // --- LÓGICA PARA OS TICKS (UTC) ---
    // A verificação de interação (✓, ×) usa UTC para ser consistente com os dados do Firestore.
    const utcDayOfWeek = now.getUTCDay(); // 0=Dom, 1=Seg, ... (baseado em UTC)
    
    // CORREÇÃO: Calcula o primeiro dia da semana (Domingo) estritamente em UTC para evitar bugs de fuso horário.
    const firstDayOfWeekUTC = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()));
    firstDayOfWeekUTC.setUTCDate(firstDayOfWeekUTC.getUTCDate() - utcDayOfWeek);
    firstDayOfWeekUTC.setUTCHours(0, 0, 0, 0);
    ...
    for (let i = 0; i < 7; i++) { // i representa o dia da semana (0=Dom, 1=Seg...)
        ...
        // Monta a data de cada tick partindo do primeiro dia da semana em UTC.
        const currentTickDate = new Date(firstDayOfWeekUTC);
        currentTickDate.setUTCDate(firstDayOfWeekUTC.getUTCDate() + i);
        
        // Converte a data do tick para a string YYYY-MM-DD (UTC) usada como chave no Firestore.
        ...
        // Verifica se houve interação para esta data UTC e aplica o estilo.
        if (interactions[dateStringUTC]) {
            dayTick.classList.add('active'); // Marca '✓'
        } else if (i < utcDayOfWeek) { 
            dayTick.classList.add('inactive'); // Marca '×' para dias passados sem interação
        }
    }
}
...
export function updateAuthUI(user, message = '', isError = false) {
    ...
    if (user) {
        ...
        if (message) {
            ...
            passwordResetMessage-div.style.display = "block";
        } else {
            passwordResetMessageDiv.style.display = 'none';
        }
    }
