...
function createTargetHTML(target, config = {}, dailyTargetsData = {}) {
    ...
    if (config.showActions) {
        ...
        // VERSÃO CORRIGIDA: Botão agora é renderizado com base na configuração, não no estado do alvo
...
export function toggleEditDeadlineForm(targetId, currentDeadline) {
    ...
    if (isVisible) {
        ...
        formDiv.innerHTML = `
            ...
            <div class="form-actions" style="display:flex; gap: 8px; margin-top: 5px;">
                <button class="btn save-deadline-btn" data-action="save-deadline" data-id="${targetId}">Salvar</button>
                <button class="btn remove-deadline-btn" data-action="remove-deadline" data-id="${targetId}" style="background-color: #e57373;">Remover Prazo</button>
                <button class="btn cancel-btn" data-action="edit-deadline" data-id="${targetId}" style="background-color: #bdbdbd;">Cancelar</button>
            </div>
        `;
        formDiv.style.display = 'block';
