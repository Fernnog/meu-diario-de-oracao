                ...
                <div class="observation-item sub-target ${isResolved ? 'resolved' : ''}">
                    <div class="sub-target-header">
                        ...
                        <div class="observation-actions">
                           ${subTargetActions}
                           <button class="btn-small demote" data-action="demote-sub-target" data-id="${parentTargetId}" data-obs-index="${index}" ${demoteButtonDisabled} title="${demoteButtonTitle}">Reverter</button>
                        </div>
                    </div>
...
export function generateViewHTML(targets, pageTitle, selectedCategories = []) {
    // 1. Agrupar alvos por categoria
    const groupedTargets = {};
    const categoriesToRender = selectedCategories.length > 0 ? selectedCategories : [...new Set(targets.map(t => t.category || 'Sem Categoria'))];
    ...
    targets.forEach(target => {
        ...
        if (groupedTargets.hasOwnProperty(category)) {
            groupedTargets[category].push(target);
        }
    });

    // 2. Construir o conteúdo HTML a partir dos grupos
    let bodyContent = '';
    const sortedCategories = Object.keys(groupedTargets).sort();

    for (const category of sortedCategories) {
        if (groupedTargets[category].length > 0) {
    ...
    if (bodyContent.trim() === '') {
        bodyContent = '<p class="empty-message">Nenhum alvo encontrado para as categorias selecionadas.</p>';
    }

    // 3. Retornar o template HTML completo com estilos e o novo botão de imprimir
    return `
        <!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><title>${pageTitle}</title>
        <style>
            body { font-family: 'Playfair Display', serif; margin: 0; padding: 20px; line-height: 1.6; color: #333; background-color: #f9f9f9; }
            ...
            h1 { color: #333; margin: 0; flex-grow: 1; font-size: 2em; }
            ...
            .target-view-item { margin-bottom: 15px; padding: 15px; border: 1px solid #f0f0f0; border-radius: 4px; background-color: #fdfdfd; transition: box-shadow 0.2s; }
            .target-view-item:hover { box-shadow: 0 1px 4px rgba(0,0,0,0.08); }
            .observations { margin-top: 10px; padding-left: 15px; border-left: 2px solid #eee; }
            .observation-item { font-size: 0.9em; }
            .view-separator-light { border: 0; border-top: 1px solid #eee; margin: 20px 0; }
            .empty-message { text-align: center; font-style: italic; color: #888; padding: 20px; }
            
            /* --- PRIORIDADE 2: Estilos para o Botão de Impressão --- */
            .print-button { padding: 10px 18px; font-size: 14px; font-family: 'Playfair Display', serif; background-color: #7a5217; color: white; border: none; border-radius: 5px; cursor: pointer; transition: background-color 0.3s; }
            .print-button:hover { background-color: #5f4012; }
            
            @media print {
                body { margin: 0; padding: 0; background-color: #fff; box-shadow: none; }
                .container { box-shadow: none; border: none; padding: 0; }
                .no-print { display: none !important; } /* Oculta o cabeçalho com o botão ao imprimir */
            }
        </style>
        </head><body>
            <div class="container">
                <div class="header no-print">
                     <!-- Botão de impressão adicionado (Prioridade 2) -->
