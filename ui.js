...
export function renderPriorityTargets(allActiveTargets, dailyTargetsData) {
    ...
    priorityTargets.forEach(target => {
        ...
        const creationTag = `<span class="creation-date-tag">Iniciado em: ${formatDateForDisplay(target.date)}</span>`;
        ...
        div.innerHTML = `
            <h3>${creationTag} ${categoryTag} ${deadlineTag} ${target.title || 'Sem Título'}</h3>
...
export function renderDailyTargets(pending, completed) {
    ...
    if (pending.length > 0) {
        pending.forEach(target => {
            ...
            div.innerHTML = `
                <h3>${creationTag} ${categoryTag} ${deadlineTag} ${target.title}</h3>
                <p class="target-details">${target.details || 'Sem detalhes.'}</p>
                ${createObservationsHTML(target.observations)}
