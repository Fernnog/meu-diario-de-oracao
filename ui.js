...
// ARQUITETURA REVISADA: A lógica de renderização dos alvos foi refatorada
// para uma função central (createTargetHTML) a fim de reduzir a duplicação
// de código e facilitar a manutenção.

// --- Funções Utilitárias de Formatação ---

/**
 * Formata um objeto Date para exibição ao usuário (ex: 27/10/2023).
 * Utiliza toLocaleDateString para respeitar o fuso horário local do usuário.
...
// --- NOVO: Template Engine de Alvos (Refatoração Arquitetônica) ---
...
function createTargetHTML(target, config = {}, dailyTargetsData = {}) {
    ...
    const archivedDatePara = config.showArchivedDate ? `<p><strong>Data Arquivamento:</strong> ${formatDateForDisplay(target.archivedDate)}</p>` : '';
    ...
    const observationsHTML = config.showObservations ? createObservationsHTML(target.observations) : '';
    ...
    return `
        <h3>${creationTag} ${categoryTag} ${deadlineTag} ${resolvedTag} ${target.title || 'Sem Título'}</h3>
        ${detailsPara}
        ${elapsedTimePara}
        ${archivedDatePara}
...
export function renderArchivedTargets(targets, total, page, perPage) {
    ...
    if (targets.length === 0) {
        ...
        targets.forEach(target => {
            ...
            // Configuração específica para alvos arquivados
            const config = {
                showCreationDate: true, // NOVO: Exibe data de criação
                showCategory: true,
                ...
                showObservations: true,
                showActions: true,
                showResolveArchivedButton: !target.resolved, // NOVO: Exibe se não estiver resolvido
