...
function createObservationsHTML(observations, parentTargetId, dailyTargetsData = {}) {
    ...
    sorted.forEach((obs, index) => {
        ...
        if (obs.isSubTarget) {
            ...
            const subTargetId = `${parentTargetId}_${index}`;
            ...
            const subTargetPrayButton = `<button class="${prayButtonClass}" data-action="pray-sub-target" data-id="${parentTargetId}" data-obs-index="${index}" ${prayButtonDisabled}>${prayButtonText}</button>`;
            ...
            // [MELHORIA APLICADA]: A ação agora chama a função para alternar o formulário inline
            const subTargetActions = !isResolved ? `
                <button class="btn-small" data-action="toggle-sub-observation-form" data-id="${parentTargetId}" data-obs-index="${index}">+ Observação</button>
            ...
            // [MELHORIA APLICADA]: Placeholder para o formulário inline.
...
export function renderDailyTargets(pending, completed, dailyTargetsData) {
    ...
    if (completed.length > 0) {
        ...
        const completedTargetsInfo = completed.map(c => ({
            id: c.id || c.targetId, // A lista de 'completed' pode vir do estado ou do DB
            title: c.title,
            isSub: c.isSubTarget || (c.targetId && c.targetId.includes('_'))
        }));

        completedTargetsInfo.forEach(targetInfo => {
            const div = document.createElement("div");
            div.className = 'target completed-target';
            div.dataset.targetId = targetInfo.id;
            div.innerHTML = `<h3>${targetInfo.isSub ? '↳ ' : ''}${targetInfo.title}</h3>`;
            container.appendChild(div);
        });
    }
}
...
/**
 * [NOVA FUNÇÃO] Exibe ou oculta o formulário para adicionar uma observação a um sub-alvo.
 * @param {string} targetId - O ID do alvo principal.
 * @param {boolean} isArchived - Se o alvo principal está arquivado. (Preservado para consistência)
...
export function toggleSubObservationForm(targetId, isArchived, obsIndex) {
    ...
    if (isVisible) {
        ...
        formDiv.innerHTML = `
            ...
            <div class="form-actions" style="text-align: right; margin-top: 5px;">
