...
// MELHORIA APLICADA: Unifica a gestão de prazos (adição, edição e remoção) em um único formulário.
...
function createTargetHTML(target, config = {}, dailyTargetsData = {}) {
    ...
    if (config.showActions) {
        ...
        // MODIFICADO: O botão "Editar Prazo" agora é visível em todos os alvos ativos.
...
/**
 * (NOVA VERSÃO) Exibe/esconde o formulário para gerenciar o prazo de um alvo.
 * Agora inclui um checkbox para habilitar/desabilitar o prazo.
 * @param {string} targetId - O ID do alvo.
 * @param {boolean} hasDeadline - Se o alvo atualmente tem um prazo.
 * @param {Date | null} currentDeadline - A data do prazo atual.
 */
export function toggleEditDeadlineForm(targetId, hasDeadline, currentDeadline) {
    const formDiv = document.getElementById(`editDeadlineForm-${targetId}`);
    if (!formDiv) return;
    const isVisible = formDiv.style.display === 'block';

    // Esconde outros formulários para evitar poluição visual
    ...
    if (isVisible) {
        ...
        const isChecked = hasDeadline ? 'checked' : '';
        const dateValue = formatDateToISO(currentDeadline); // `formatDateToISO` lida com datas nulas.
        const containerDisplay = hasDeadline ? 'block' : 'none';

        // Gera o HTML do formulário com o checkbox e o campo de data condicional
        formDiv.innerHTML = `
            <div class="checkbox-container" style="justify-content: flex-start; margin-bottom: 15px;">
                <input type="checkbox" id="deadlineToggle-${targetId}" ${isChecked} 
                       onchange="document.getElementById('deadlineDateContainer-${targetId}').style.display = this.checked ? 'block' : 'none';">
                <label for="deadlineToggle-${targetId}">Este alvo tem prazo de validade?</label>
            </div>

            <div id="deadlineDateContainer-${targetId}" style="display: ${containerDisplay};">
                <label for="deadlineInput-${targetId}">Prazo de Validade:</label>
                <input type="date" id="deadlineInput-${targetId}" value="${dateValue}" style="width: 95%;">
            </div>

            <button class="btn save-deadline-btn" data-action="save-deadline" data-id="${targetId}">Salvar Alterações</button>
        ...
        formDiv.style.display = 'block';
        formDiv.querySelector('input[type="checkbox"]')?.focus();
    }
}
