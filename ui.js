...
// **MODIFICADO: Função 'renderPriorityTargets' com as novas funcionalidades**
export function renderPriorityTargets(allActiveTargets, dailyTargetsData) {
    ...
    // Lógica para a mensagem de painel vazio
    if (priorityTargets.length === 0) {
        section.style.display = 'block'; // Garante que a seção esteja visível para a mensagem
    ...
    // Cria um Set com os IDs dos alvos já orados hoje para busca rápida
    ...
    priorityTargets.forEach(target => {
        ...
        const buttonText = hasBeenPrayedToday ? 'Orado hoje!' : 'Orei!';
        ...
        div.innerHTML = `
            ...
            <div class="priority-target-actions">
                 <button class="btn pray-button" data-action="pray-priority" data-id="${target.id}" ${buttonDisabled}>${buttonText}</button>
            </div>
        `;
