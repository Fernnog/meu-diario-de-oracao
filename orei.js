...
// MODIFICADO: Adicionadas novas categorias (para consistência na edição se implementada aqui)
...
function rehydrateTargets(targets) {
     return targets.map((target) => {
        ...
        fieldsToConvert.forEach(field => {
         ...
         // Ensure category is string or null (important for filtering/display)
...
function applyFiltersAndRenderMainReport() {
    ...
    filteredTargetsForReport = allTargetsForReport.filter(target => {
        ...
        if (searchTerm) {
             // Include category in search on this page as well
             ...
             if (!titleMatch && !detailsMatch && !categoryMatch) return false; // Add category check
        }
        return true;
    });
...
function renderMainReportList() {
    ...
    itemsToDisplay.forEach(target => {
        ...
        switch (target.status) {
         ...
         // Adiciona tag de categoria se existir
         let categoryTag = '';
         if (target.category) {
             // Usando um estilo simples para a tag de categoria aqui
             categoryTag = `<span style="background-color: #C71585; color: #fff; padding: 2px 6px; border-radius: 3px; font-size: 0.7em; margin-right: 8px; vertical-align: middle;">${target.category}</span>`;
         }
        ...
        itemDiv.innerHTML = `
            <h3>${statusLabel} ${categoryTag} ${target.title || 'Sem Título'}</h3>
