...
// ALTERAÇÃO: A função agora retorna apenas o conteúdo interno (cabeçalho e parágrafos).
function renderObservations(observations) {
    ...
    // Retorna apenas o conteúdo, sem o div container.
    return `
        <h4>Atualizações:</h4>
...
function refreshDailyTargets() {
   ...
   // Se todos os alvos foram exibidos, reseta o histórico
    ...
    selectedTargets.forEach((target, index) => {
        ...
        // Construindo o HTML inicial para o alvo
        const deadlineTag = target.hasDeadline ? `<span class="deadline-tag ${isDateExpired(target.deadlineDate) ? 'expired' : ''}">Prazo: ${formatDateForDisplay(target.deadlineDate)}</span>` : '';
        let contentHTML = `
            <h3>${deadlineTag} ${target.title}</h3>
            <p>${target.details}</p>
        ...
        // ALTERAÇÃO: Chama renderObservations e envolve o resultado no div container.
        const observationsHTML = renderObservations(target.observations);
        if (observationsHTML) {
            dailyDiv.innerHTML += `<div class="observations-list">${observationsHTML}</div>`;
        }
