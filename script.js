...
function rehydrateTargets(targets) {
    return targets.map(target => {
        if (target.date && !(target.date instanceof Date)) target.date = target.date.toDate(); // <---- MODIFICADO para .toDate() e verificação instanceof
        if (target.archivedDate && !(target.archivedDate instanceof Date)) target.archivedDate = target.archivedDate.toDate(); // <---- MODIFICADO para .toDate() e verificação instanceof
        if (target.deadlineDate && !(target.deadlineDate instanceof Date)) target.deadlineDate = target.deadlineDate.toDate(); // <---- MODIFICADO para .toDate() e verificação instanceof
        if (target.observations) {
            target.observations.forEach(obs => {
                if (obs.date && !(obs.date instanceof Date)) obs.date = obs.date.toDate(); // <---- MODIFICADO para .toDate() e verificação instanceof
             });
        }
        return target;
    });
...
async function fetchPrayerTargets(uid) {
    ...
    targetsSnapshot.forEach((doc) => {
        const data = doc.data();
        prayerTargets.push({...data, id: doc.id, date: data.date.toDate() }); // <------- ADICIONE .toDate() AQUI
    });
...
async function fetchArchivedTargets(uid) {
    ...
    archivedSnapshot.forEach((doc) => {
        const data = doc.data();
        archivedTargets.push({...data, id: doc.id, date: data.date.toDate(), archivedDate: data.archivedDate.toDate() }); // <------- ADICIONE .toDate() AQUI para date e archivedDate
    });
...
async function saveObservation(targetId) {
    ...
    if (observationText !== "") {
        let observationDate; // Declare observationDate outside the if/else

        if (observationDateValue) {
            observationDate = Timestamp.fromDate(new Date(observationDateValue + "T00:00:00Z")); // Usar Timestamp.fromDate para data do input, forçando UTC
        } else {
            observationDate = Timestamp.fromDate(new Date()); // Usar Timestamp.fromDate para data atual, por padrão, já é local, converte para UTC ao salvar
        }
        ...
        try {
            ...
            if (targetDoc.exists()) {
                ...
                updatedObservations.push({ date: observationDate, observation: observationText }); // Salva observationDate como Timestamp
...
document.getElementById("prayerForm").addEventListener("submit", async (e) => {
    ...
    const deadlineDateInput = document.getElementById("deadlineDate").value; // Renomeei para evitar confusão
    const dateInput = document.getElementById("date").value; // Mantive este nome
    ...
    const newTarget = {
        ...
        date: Timestamp.fromDate(new Date(dateInput + "T00:00:00Z")), // <------- MODIFICADO para Timestamp e forçando UTC
        resolved: false,
        observations: [],
        hasDeadline: hasDeadline,
        deadlineDate: hasDeadline ? Timestamp.fromDate(new Date(deadlineDateInput + "T00:00:00Z")) : null, // <------- MODIFICADO para Timestamp e forçando UTC
        ...
        if (user) {
            ...
            const targetDoc = await getDoc(targetRef); // Use getDoc here
             ...
             if (targetDoc.exists()) {
                ...
                const resolvedTargetData = {...targetDoc.data(), resolved: true, archivedDate: Timestamp.fromDate(new Date())}; // <---- MODIFICADO para Timestamp
...
async function archiveTarget(targetId) {
    try {
        ...
        if (user) {
            ...
            if (targetDoc.exists()) {
                //Não precisa do userId aqui, já estamos na coleção do usuário correto
                 const archivedTargetData = {...targetDoc.data(), archivedDate: Timestamp.fromDate(new Date())}; // <---- MODIFICADO para Timestamp
                const archivedRef = doc(collection(db, "users", user.uid, "archivedTargets"));// Usa o UID do usuário
...
async function editDeadline(targetId) {
    ...
    try {
        ...
        if (user) {
            const targetRef = doc(db, "users", user.uid, "prayerTargets", targetId);
            await updateDoc(targetRef, { deadlineDate: Timestamp.fromDate(convertToISO(newDeadline) + "T00:00:00Z") }); // <---- MODIFICADO para Timestamp e forçando UTC
...
// Script de Migração de Datas Existentes para Timestamps (Execute apenas uma vez e comente/remova depois!)
/*
async function migrateDatesToTimestamps(userId) {
    ...
    snapshot.forEach(async (doc) => {
        ...
        if (typeof data.date === "string") {
            console.log(`Migrando documento: ${doc.id}, data antiga: ${data.date}`);
            const correctedDate = new Date(data.date + "T00:00:00Z"); // Interpreta data string como UTC
            // Não precisa adicionar dias, pois estamos interpretando como UTC e convertendo para Timestamp
            await updateDoc(doc.ref, {
                date: Timestamp.fromDate(correctedDate),
                deadlineDate: data.deadlineDate ? Timestamp.fromDate(new Date(data.deadlineDate  + "T00:00:00Z")) : null, // Garante que deadlineDate também seja Timestamp se existir
            });
            count++;
            console.log(`Documento: ${doc.id} migrado para Timestamp.`);
        }
    });
    alert(`${count} datas de alvos migradas para Timestamps!`);
}

// Para executar a migração (use apenas UMA VEZ e depois remova ou comente este código):
onAuthStateChanged(auth, (user) => {
    if (user) {
        migrateDatesToTimestamps(user.uid).then(() => console.log("Migração de datas completa!"));
    }
});
*/