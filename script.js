...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.recalcularPlanoPeriodoOriginal = function(index) {
        ...
        // Filtra os dias do plano para excluir dias passados não lidos
        const diasFuturosNaoLidos = plano.diasPlano.filter(dia => {
            const dataDia = new Date(dia.data);
            dataDia.setHours(0, 0, 0, 0);
            return dataDia >= hoje || dia.lido; // Mantém dias lidos e dias futuros
        });

        // Encontra o índice do primeiro dia não lido no array filtrado para começar o recalculo a partir dali
        const firstNotLidoIndex = diasFuturosNaoLidos.findIndex(dia => !dia.lido);
        if (firstNotLidoIndex === -1) {
            alert("Plano já concluído ou não há dias pendentes para recalcular.");
            return;
        }

        let paginasLidasAteAgora = diasFuturosNaoLidos.slice(0, firstNotLidoIndex).reduce((sum, dia) =>
        sum + (dia.lido ? dia.paginas : 0), 0);
        const paginasRestantes = plano.totalPaginas - paginasLidasAteAgora;

        // Considera apenas os dias futuros e não lidos para redistribuição
        const diasRestantes = diasFuturosNaoLidos.slice(firstNotLidoIndex);

        if (diasRestantes.length === 0) {
            alert("Não há dias restantes no plano para recalcular após remover datas atrasadas.");
        ...
        if (firstNotLidoIndex === 0) {
            paginaAtual = plano.paginaInicio + paginasLidasAteAgora; // Inicia após as páginas já lidas
        } else if (firstNotLidoIndex > 0) {
            paginaAtual = diasFuturosNaoLidos[firstNotLidoIndex - 1].paginaFimDia + 1;
        }
        ...
        // Atualiza o plano original com os dias recalculados (apenas os dias redistribuídos, mantendo os anteriores)
        // Mantém os dias lidos originais e substitui os dias futuros não lidos pelos recalculados
        const diasLidosOriginais = plano.diasPlano.slice(0, plano.diasPlano.length - diasRestantes.length); // Pega os dias originais até o ponto de recalculo
        const diasRecalculados = diasRestantes; // Usa os diasRestantes que já são os dias futuros recalculados

        planos[index].diasPlano = [...diasLidosOriginais, ...diasRecalculados];
