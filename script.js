...
relatorioBtn.addEventListener('click', () => { // 'relatorioBtn' agora lida com a exportação (antigo 'Relatório Excel')
    ...
    try { // ADDED: Error handling block
        const dataAtual = new Date().toISOString().slice(0, 10).replace(/-/g, '');
        const nomeArquivo = `${dataAtual}_Compras_no_mercado.xlsx`;
        ...
        const wbout = XLSX.write(wb, { bookType: 'xlsx', mimeType: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' }); // Get workbook data

        const blob = new Blob([s2ab(wbout)], {type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}); // Create Blob
        ...
        mostrarFeedbackSucesso('Lista de compras exportada com sucesso!');
        console.log('XLSX file exported successfully:', nomeArquivo); // ADDED: Success log

    } catch (error) { // ADDED: Catch errors
        console.error('Error exporting XLSX file:', error); // ADDED: Error log
        mostrarFeedbackErro('Erro ao exportar a lista de compras. Verifique o console para mais detalhes.');
    }
});

// Utility function to convert string to ArrayBuffer (required for XLSX.write with type 'binary')
function s2ab(s) {
...
limparListaBtn.addEventListener('click', () => {
    if (confirm('Deseja exportar os dados antes de limpar?')) {
        exportarBtn.click(); // Changed to exportarBtn (which is relatorioBtn)
    }
