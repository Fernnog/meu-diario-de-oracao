...
function atualizarLista(filtrados = compras) {
    ...
    filtrados.forEach((item, index) => {
        ...
        const valorZeroClass = item.valorUnitario === 0 ? 'excluir-item-vermelho' : 'excluir-item-verde';
        li.innerHTML = `${item.quantidade}x ${item.descricao} - R$ ${item.valorUnitario.toFixed(2).replace('.', ',')} (${item.categoria}) `;
        li.innerHTML += `<button class="excluir-item ${valorZeroClass}" data-index="${index}">🗑️</button>`; // Usa o ícone de lixeira e classe condicional
        ...
        li.addEventListener('click', (event) => {
            ...
            if (!event.target.classList.contains('excluir-item') &&
                !event.target.classList.contains('excluir-item-verde') &&
                !event.target.classList.contains('excluir-item-vermelho')) {
                editarItem(index);
            }
        });
...
exportarBtn.addEventListener('click', () => {
    ...
    const nomeArquivo = `${dataAtual}_ListaCompras.xlsx`;
    XLSX.writeFile(wb, nomeArquivo);

    // Descomente para exportar em JSON também, se necessário, ou remova este bloco se XLSX for suficiente
    // const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(compras));
    // const downloadAnchor = document.createElement('a');
    // downloadAnchor.setAttribute("href", dataStr);
    // downloadAnchor.setAttribute("download", nomeArquivo.replace('.xlsx', '.json')); // Altera a extensão para .json
