// script.js (Orquestrador Principal da Aplicação - Versão Corrigida e Completa)
// ARQUITETURA CORRIGIDA: Usa o módulo auth.js para toda a gestão de autenticação,
// resolvendo o TypeError e centralizando as responsabilidades.

// --- MÓDULOS ---
import { initializeAuth, handleSignOut, signUpWithEmailPassword, signInWithEmailPassword, resetPassword } from './auth.js';
...
// =================================================================
// === LÓGICA DE AUTENTICAÇÃO E FLUXO DE DADOS (Refatorado) ===
// =================================================================

/**
 * Função de callback que é passada para o módulo de autenticação.
 * Ela dita o que acontece na aplicação quando o estado do usuário muda.
...
document.addEventListener('DOMContentLoaded', () => {
    // CORRIGIDO: A inicialização agora é feita através do módulo auth.js,
    // passando a função que deve ser executada quando o estado de auth mudar.
    ...
    document.getElementById('btnLogout').addEventListener('click', () => {
        handleSignOut().catch(error => console.error("Erro ao sair:", error));
    });
        ...
        switch(action) {
            ...
            case 'pray-sub-target': {
                try {
                    ...
                    if (parentTarget) { state.dailyTargets.completed.push({ targetId: subTargetId, isSubTarget: true }); }
            ...
            // Adicionado para suportar o formulário inline de sub-observação
            case 'toggle-sub-observation-form': {
                UI.toggleSubObservationForm(id, isArchived, parseInt(obsIndex));
                break;
            }
            
            // Adicionado para salvar os dados do formulário inline
