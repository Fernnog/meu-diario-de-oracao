...
// VERSÃO APRIMORADA: Inclui atualizações otimistas para uma melhor experiência do usuário e a nova lógica para gerenciar sub-alvos.
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.body.addEventListener('click', async e => {
        ...
        // --- NOVA LÓGICA PARA SUB-ALVOS (PRIORIDADES 1 E 2) ---
        ...
        if (subTargetActions.includes(action)) {
            ...
            // Cria uma cópia profunda para permitir a reversão em caso de erro
            ...
            if (action === 'promote-observation') {
                ...
                updatedObservation = { ...originalObservation, isSubTarget: true, subTargetTitle: subTargetTitle, subTargetStatus: 'pending' };
            } else if (action === 'resolve-sub-target') {
                updatedObservation = { ...originalObservation, subTargetStatus: 'resolved' };
            } else if (action === 'edit-sub-target-title') {
                const newTitle = prompt("Editar título do sub-alvo:", originalObservation.subTargetTitle);
                if (!newTitle) return;
                updatedObservation = { ...originalObservation, subTargetTitle: newTitle };
                ...
                delete updatedObservation.isSubTarget;
                delete updatedObservation.subTargetTitle;
                delete updatedObservation.subTargetStatus;
            }
            
            // (*** INÍCIO DA CORREÇÃO CRÍTICA ***)
            // JSON.stringify converteu a Data para uma string. Devemos convertê-la de volta!
            if (updatedObservation.date && typeof updatedObservation.date === 'string') {
                updatedObservation.date = new Date(updatedObservation.date);
            }
            // (*** FIM DA CORREÇÃO CRÍTICA ***)

            // UI Otimista: Atualiza o estado local e re-renderiza imediatamente
            target.observations[observationIndex] = updatedObservation;
            applyFiltersAndRender(panelId);

            try {
                // Sincroniza com o backend
                await Service.updateObservationInTarget(state.user.uid, id, isArchived, observationIndex, updatedObservation);
            } catch (error) {
                ...
                // Reverte a alteração no estado local e re-renderiza
                target.observations[observationIndex] = originalObservation;
                applyFiltersAndRender(panelId);
            }
            return; // Encerra o fluxo aqui para não cair no switch-case abaixo
        }
        
        // --- LÓGICA EXISTENTE PARA AÇÕES PRINCIPAIS ---
