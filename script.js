...
function rehydrateTargets(targets) {
    return targets.map(target => {
        if (target.date) target.date = new Date(target.date + "T00:00:00Z"); // Interpret date strings as UTC on rehydration
        if (target.archivedDate) target.archivedDate = new Date(target.archivedDate + "T00:00:00Z"); // Interpret archivedDate strings as UTC
        if (target.deadlineDate) target.deadlineDate = new Date(target.deadlineDate + "T00:00:00Z"); // Interpret deadlineDate strings as UTC
        if (target.observations) target.observations.forEach(obs => obs.date = new Date(obs.date + "T00:00:00Z")); // Interpret observation dates as UTC
        return target;
    });
}
...
async function saveObservation(targetId) {
    ...
    if (observationText !== "") {
        let observationDate = observationDateValue ? formatDateToISO(new Date(observationDateValue + "T00:00:00Z")) : formatDateToISO(new Date(new Date().getTime() + new Date().getTimezoneOffset() * 60000)); // Force UTC interpretation if date is provided, otherwise use current date in ISO format (already using UTC functions in formatDateToISO)
...
document.getElementById("prayerForm").addEventListener("submit", async (e) => {
    ...
    const deadlineDate = hasDeadline
        ? formatDateToISO(new Date(document.getElementById("deadlineDate").value + "T00:00:00Z")) // Force UTC interpretation
        : null;
    ...
    const newTarget = {
        ...
        date: formatDateToISO(new Date(document.getElementById("date").value + "T00:00:00Z")), // Force UTC interpretation
        resolved: false,
        observations: [],
