...
async function fetchPrayerTargets(uid) {
    ...
    targetsSnapshot.forEach((doc) => {
        ...
        if (data.deadlineDate) data.deadlineDate = data.deadlineDate.toDate();
        if (data.observations) {
            data.observations.forEach(obs => {
                if (obs.date) obs.date = obs.date.toDate(); // Convert observation dates too
            });
        }
        prayerTargets.push({...data, id: doc.id});
    });
...
async function fetchArchivedTargets(uid) {
    ...
    archivedSnapshot.forEach((doc) => {
        ...
        // Convert Firestore Timestamps back to JavaScript Date objects
        if (data.date) data.date = data.date.toDate();
        if (data.archivedDate) data.archivedDate = data.archivedDate.toDate(); //Assuming archivedDate is also a Timestamp
        if (data.deadlineDate) data.deadlineDate = data.deadlineDate.toDate();
        if (data.observations) {
            data.observations.forEach(obs => {
                if (obs.date) obs.date = obs.date.toDate(); // Convert observation dates too
        ...
        archivedTargets.push({...data, id: doc.id});
    });
...
async function saveObservation(targetId) {
    ...
    if (observationText !== "") {
        let observationDate = observationDateValue ? new Date(observationDateValue + "T00:00:00Z") : new Date(); // Force UTC interpretation if date is provided, otherwise current UTC time
        ...
        try {
            ...
            if (targetDoc.exists()) {
                ...
                updatedObservations.push({ date: Timestamp.fromDate(observationDate), observation: observationText }); // Save observation date as Timestamp
...
document.getElementById("prayerForm").addEventListener("submit", async (e) => {
    ...
    const deadlineDateValue = document.getElementById("deadlineDate").value;
    const dateInputValue = document.getElementById("date").value;

    // Convert date input values to Date objects, then to Firestore Timestamps, forcing UTC
    const date = new Date(dateInputValue + "T00:00:00Z"); // Force UTC interpretation
    const dateTimestamp = Timestamp.fromDate(date);

    let deadlineDateTimestamp = null;
    if (hasDeadline && deadlineDateValue) {
        const deadlineDate = new Date(deadlineDateValue + "T00:00:00Z"); // Force UTC interpretation
        deadlineDateTimestamp = Timestamp.fromDate(deadlineDate);
    }
    ...
    const newTarget = {
        ...
        deadlineDate: deadlineDateTimestamp, // Save as Timestamp (or null)
        ...
        if (user) {
            ...
            const targetDoc = await getDoc(targetRef); // Use getDoc here
             ...
             if (targetDoc.exists()) {
                ...
                const resolvedTargetData = {...targetDoc.data(), resolved: true, archivedDate: Timestamp.fromDate(new Date())}; // archivedDate as Timestamp
...
async function archiveTarget(targetId) {
    try {
        ...
        if (user) {
            ...
            if (targetDoc.exists()) {
                //Não precisa do userId aqui, já estamos na coleção do usuário correto
                 const archivedTargetData = {...targetDoc.data(), archivedDate: Timestamp.fromDate(new Date())}; // archivedDate as Timestamp
                const archivedRef = doc(collection(db, "users", user.uid, "archivedTargets"));// Usa o UID do usuário
...
async function editDeadline(targetId) {
    ...
    try {
        ...
        if (user) {
            const targetRef = doc(db, "users", user.uid, "prayerTargets", targetId);
            await updateDoc(targetRef, { deadlineDate: Timestamp.fromDate(convertToUTCDate(newDeadline)) }); // Save deadline as Timestamp in UTC
...
// ==== INÍCIO SEÇÃO - SCRIPT DE MIGRAÇÃO DE DATAS (OPCIONAL - EXECUTE UMA VEZ) ====
async function migrateDates() {
    const userId = auth.currentUser.uid; // Certifique-se de que o usuário está logado para ter o UID
    if (!userId) {
        alert("Usuário não autenticado. Migração de datas não pode ser iniciada.");
        return;
    }
    ...
    snapshot.forEach(async (doc) => {
        ...
        if (typeof data.date === "string") {
            console.log(`Migrando data para alvo: ${data.title}, data atual: ${data.date}`);
            const correctedDate = new Date(data.date + "T00:00:00Z"); // Interpreta como UTC
            // Se as datas estavam sendo salvas com um offset de 2 dias no futuro, descomente a linha abaixo
            // correctedDate.setDate(correctedDate.getDate() - 2); // Subtrai 2 dias SE necessário
            updateData.date = Timestamp.fromDate(correctedDate);
            console.log(`Data corrigida para: ${correctedDate.toISOString()}`);
        }

        if (data.deadlineDate && typeof data.deadlineDate === "string") {
            console.log(`Migrando deadlineDate para alvo: ${data.title}, deadlineDate atual: ${data.deadlineDate}`);
            const correctedDeadlineDate = new Date(data.deadlineDate + "T00:00:00Z"); // Interpreta como UTC
            // Se os prazos estavam com offset de 2 dias, descomente:
            // correctedDeadlineDate.setDate(correctedDeadlineDate.getDate() - 2); // Subtrai 2 dias SE necessário
            updateData.deadlineDate = Timestamp.fromDate(correctedDeadlineDate);
            console.log(`deadlineDate corrigida para: ${correctedDeadlineDate.toISOString()}`);
        }

        if (data.observations && Array.isArray(data.observations)) {
            const updatedObservations = data.observations.map(obs => {
                if (typeof obs.date === "string") {
                    console.log(`Migrando data de observação para alvo: ${data.title}, data obs: ${obs.date}`);
                    const correctedObsDate = new Date(obs.date + "T00:00:00Z"); // Interpreta como UTC
                    // Se as datas de observação estavam com offset de 2 dias, descomente:
                    // correctedObsDate.setDate(correctedObsDate.getDate() - 2); // Subtrai 2 dias SE necessário
                    obs.date = Timestamp.fromDate(correctedObsDate);
                    console.log(`Data de observação corrigida para: ${correctedObsDate.toISOString()}`);
                }
                return obs;
            });
        ...
        if (Object.keys(updateData).length > 0) {
            ...
            console.log(`Nenhuma migração de data necessária para o documento: ${doc.id}`);
        }
    });

    alert("Migração de datas completa (verifique o console para detalhes). IMPORTANTE: Execute este script apenas UMA VEZ e faça backup dos dados antes!");
}
// ==== FIM SEÇÃO - SCRIPT DE MIGRAÇÃO DE DATAS ====