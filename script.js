...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // NOVO: Listeners para filtro de data
    ['Main', 'Archived'].forEach(panelSuffix => {
        const panelId = panelSuffix.toLowerCase() + 'Panel';
        const startInput = document.getElementById(`filterStartDate${panelSuffix}`);
        const endInput = document.getElementById(`filterEndDate${panelSuffix}`);
        const clearButton = document.getElementById(`clearDateFilter${panelSuffix}`);
        
        startInput?.addEventListener('change', e => {
            state.filters[panelId].startDate = e.target.value;
            state.pagination[panelId].currentPage = 1;
            applyFiltersAndRender(panelId);
        });
        endInput?.addEventListener('change', e => {
            state.filters[panelId].endDate = e.target.value;
            state.pagination[panelId].currentPage = 1;
            applyFiltersAndRender(panelId);
        });
        clearButton?.addEventListener('click', () => {
            startInput.value = '';
            endInput.value = '';
            state.filters[panelId].startDate = null;
            state.filters[panelId].endDate = null;
            state.pagination[panelId].currentPage = 1;
            applyFiltersAndRender(panelId);
        });
    });
    ...
    document.body.addEventListener('click', async e => {
        ...
        // Combina todas as listas de alvos para encontrar o alvo correto
        const allTargets = [...state.prayerTargets, ...state.archivedTargets, ...state.resolvedTargets];
        const target = allTargets.find(t => t.id === id);

        switch(action) {
            case 'pray': await handlePray(id); break;
            case 'pray-priority': await handlePray(id); break;
            ...
            case 'download-archived': {
                ...
                let htmlContent = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Histórico</title></head><body>
                    <h1>Histórico do Alvo de Oração</h1><hr>
                    <p><b>Título:</b> ${sanitize(target.title)}</p>
                    <p><b>Categoria:</b> ${target.category || 'Não definida'}</p>
                    <p><b>Data de Criação:</b> ${formatDateForDoc(target.date)}</p>`;

                if (target.hasDeadline) htmlContent += `<p><b>Prazo de Validade:</b> ${formatDateForDoc(target.deadlineDate)}</p>`;
                htmlContent += `<h3>Descrição Inicial</h3><p>${sanitize(target.details).replace(/\n/g, '<br>') || 'Nenhuma.'}</p>`;

                if (target.observations && target.observations.length > 0) {
                    htmlContent += `<h3>Observações Posteriores</h3>`;
                    [...target.observations].sort((a, b) => a.date.getTime() - b.date.getTime()).forEach(obs => {
                        htmlContent += `<p><b>[${formatDateForDoc(obs.date)}]:</b> ${sanitize(obs.text).replace(/\n/g, '<br>')}</p>`;
                    });
                }

                htmlContent += `<hr>`;
                if (target.resolved) htmlContent += `<p><b>Status:</b> RESPONDIDO em ${formatDateForDoc(target.resolutionDate)}</p>`;
                else htmlContent += `<p><b>Status:</b> ARQUIVADO em ${formatDateForDoc(target.archivedDate)}</p>`;
                htmlContent += `</body></html>`;
                
                const blob = new Blob([htmlContent], { type: 'application/msword' });
                const url = URL.createObjectURL(blob);
                const a = document.createElement('a');
                const safeTitle = (target.title || 'Alvo').replace(/[^a-z0-9]/gi, '_').toLowerCase();
                a.href = url;
                a.download = `historico_${safeTitle}.doc`;
            ...
            case 'toggle-priority': {
                ...
                UI.renderPriorityTargets(state.prayerTargets, state.dailyTargets);
                try {
                    await Service.updateTargetField(state.user.uid, id, !!target.archived || !!target.resolved, { isPriority: newStatus });
                } catch (error) {
                    console.error("Falha ao atualizar prioridade:", error);
                    alert("Ocorreu um erro. A alteração foi desfeita.");
                    target.isPriority = !newStatus;
                    applyFiltersAndRender('mainPanel');
                    UI.renderPriorityTargets(state.prayerTargets, state.dailyTargets);
                }
