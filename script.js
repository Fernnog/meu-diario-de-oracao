...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.body.addEventListener('click', async e => {
        ...
        switch(action) {
            ...
            case 'edit-deadline': 
                UI.toggleEditDeadlineForm(id, target?.deadlineDate, target?.hasDeadline); 
                break;

            case 'save-deadline': { 
                const newHasDeadline = document.getElementById(`hasDeadlineCheckbox-${id}`).checked;
                let newDeadlineDate = null;

                if (newHasDeadline) {
                    const newDeadlineStr = document.getElementById(`deadlineInput-${id}`).value;
                    if (!newDeadlineStr) {
                        return alert("Por favor, selecione uma data ou desmarque a opção de prazo.");
                    }
                    newDeadlineDate = new Date(newDeadlineStr + 'T12:00:00Z');
                }

                // UI Otimista: Salva o estado antigo e atualiza o estado local
                const oldHasDeadline = target.hasDeadline;
                const oldDeadlineDate = target.deadlineDate;
                target.hasDeadline = newHasDeadline;
                target.deadlineDate = newDeadlineDate;
                
                UI.toggleEditDeadlineForm(id, null, false); // Fecha o formulário
                applyFiltersAndRender(panelId); // Re-renderiza a UI com a mudança

                try {
                    // Persiste a mudança no Firestore
                    await Service.updateTargetField(state.user.uid, id, isArchived, { 
                        hasDeadline: newHasDeadline, 
                        deadlineDate: newDeadlineDate 
                    });
                    alert("Prazo atualizado com sucesso!");
                } catch(error) {
                    // Se der erro, reverte a mudança e informa o usuário
                    alert("Falha ao salvar o prazo. A alteração será desfeita.");
