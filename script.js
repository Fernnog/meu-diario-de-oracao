...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.body.addEventListener('click', async e => {
        ...
        switch(action) {
            ...
            // --- INÍCIO DA ALTERAÇÃO ---
            // Substitui a lógica de 'prompt' por uma que mostra/esconde um formulário.
            case 'toggle-sub-observation-form': {
                // A ação agora apenas chama a função da UI para exibir ou ocultar o formulário.
                UI.toggleSubObservationForm(id, isArchived, parseInt(obsIndex));
                break;
            }

            // Nova ação para salvar os dados do formulário inline.
            case 'save-sub-observation': {
                const text = document.getElementById(`subObsText-${id}-${obsIndex}`).value.trim();
                if (!text) {
                    showToast("O texto da observação não pode estar vazio.", "error");
                    return;
                }
                
                const newSubObservation = { text, date: new Date() };

                // UI Otimista: Adiciona à lista localmente e atualiza a tela.
                ...
                applyFiltersAndRender(panelId); // Renderiza o painel para mostrar a nova sub-observação.

                try {
                    // Chama o serviço para persistir a alteração no Firestore.
                    ...
                    // Em caso de erro, desfaz a alteração na UI e notifica o usuário.
                    showToast("Erro ao salvar. A alteração foi desfeita.", "error");
