...
async function loadDataForUser(user) {
    try {
        ...
        UI.renderPriorityTargets(state.prayerTargets); // **NOVO: Renderiza o painel de prioridades**
...
async function handleAddNewTarget(event) {
    ...
    // **NOVO: Captura o estado do checkbox de prioridade**
    ...
    const newTarget = { 
        ...
        observations: [], 
        resolved: false,
        isPriority: isPriority // **NOVO: Adiciona o campo ao objeto do alvo**
    };
    try {
...
async function handlePray(targetId) {
    ...
    try {
        ...
        const [perseveranceData, weeklyData, dailyTargetsData] = await Promise.all([
            Service.loadPerseveranceData(state.user.uid), Service.loadWeeklyPrayerData(state.user.uid), Service.loadDailyTargets(state.user.uid, state.prayerTargets)
        ]);
        state.perseveranceData = perseveranceData; state.weeklyPrayerData = weeklyData; state.dailyTargets = dailyTargetsData;
        UI.renderDailyTargets(state.dailyTargets.pending, state.dailyTargets.completed);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.body.addEventListener('click', async e => {
        ...
        switch(action) {
            ...
            // **MELHORIA DE UX: LÓGICA DE ATUALIZAÇÃO OTIMISTA (INSTANTÂNEA)**
            case 'toggle-priority': {
                ...
                applyFiltersAndRender('mainPanel'); 
                UI.renderPriorityTargets(state.prayerTargets);
