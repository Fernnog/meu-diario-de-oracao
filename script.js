...
import { getAuth, signOut, onAuthStateChanged, GoogleAuthProvider, signInWithPopup } from "https://www.gstatic.com/firebasejs/9.22.2/firebase-auth.js"; // Import GoogleAuthProvider and signInWithPopup, REMOVE email/password methods
...
function updateAuthUI(user) {
    const btnGoogleLogin = document.getElementById('btnGoogleLogin'); // Google Login button
    const btnLogout = document.getElementById('btnLogout');
    const authStatus = document.getElementById('authStatus');
    const authStatusContainer = document.querySelector('.auth-status-container'); // Obtém o container

    if (user) {
        authStatus.textContent = "Usuário autenticado: " + user.email + " (via Google)"; // Indica login Google
...
function renderArchivedTargets() {
    ...
    const endIndex = startIndex + currentArchivedPage;
    const targetsToDisplay = filteredTargets.slice(startIndex, endIndex);
...
document.addEventListener('DOMContentLoaded', () => {
    const btnGoogleLogin = document.getElementById('btnGoogleLogin'); // Get Google Login button
    const btnLogout = document.getElementById('btnLogout');
