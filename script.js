...
function handleLogoutState() {
    ...
    // Esconde o navegador flutuante no logout
    updateFloatingNavVisibility(state);
}
    ...
    document.body.addEventListener('click', async e => {
        ...
        switch(action) {
            ...
            case 'download-archived': {
                ...
                // Função auxiliar para formatar datas, evitando 'Data Inválida'
                const formatDateForTxt = (date) => {
                    if (!date || !(date instanceof Date) || isNaN(date.getTime())) return 'N/A';
                    return date.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', year: 'numeric' });
                };

                let content = `Histórico do Alvo de Oração\n`;
                content += `========================================\n\n`;
                content += `Título: ${target.title}\n`;
                content += `Categoria: ${target.category || 'Não definida'}\n`;
                content += `Data de Criação: ${formatDateForTxt(target.date)}\n`;
                
                if (target.hasDeadline) {
                    content += `Prazo de Validade: ${formatDateForTxt(target.deadlineDate)}\n`;
                }

                content += `\n--- Descrição Inicial ---\n${target.details || 'Nenhuma.'}\n`;

                if (target.observations && target.observations.length > 0) {
                    content += `\n--- Observações Posteriores ---\n`;
                    // Ordena as observações por data para o relatório
                    [...target.observations]
                        .sort((a, b) => a.date.getTime() - b.date.getTime())
                        .forEach(obs => {
                            content += `\n[${formatDateForTxt(obs.date)}]: ${obs.text}\n`;
                        });
                }

                content += `\n========================================\n`;
                if (target.resolved) {
                    content += `Status: RESPONDIDO em ${formatDateForTxt(target.resolutionDate)}\n`;
                } else {
                    content += `Status: ARQUIVADO em ${formatDateForTxt(target.archivedDate)}\n`;
                }
                ...
                a.href = url;
                a.download = `historico_${safeTitle}.txt`;
