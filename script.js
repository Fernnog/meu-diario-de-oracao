...
function formatDateForDisplay(dateInput) {
    ...
    else if (dateInput instanceof Date) { dateToFormat = dateInput; }
    else {
...
function rehydrateTargets(targets) {
    ...
    return targets.map((target, index) => {
        ...
        const fieldsToConvert = ['date', 'deadlineDate', 'lastPresentedDate', 'resolutionDate', 'archivedDate']; // Include archivedDate if needed elsewhere
        fieldsToConvert.forEach(field => {
            ...
            else if (originalValue instanceof Date && !isNaN(originalValue)) { rehydratedTarget[field] = originalValue; }
...
function renderArchivedTargets() {
    ...
    filteredAndPagedArchivedTargets.sort((a, b) => (b.date || 0) - (a.date || 0)); // Mais recentes primeiro
...
function renderResolvedTargets() {
    ...
    filteredAndPagedResolvedTargets.sort((a, b) => (b.resolutionDate || 0) - (a.resolutionDate || 0)); // Mais recentes primeiro
...
async function loadPerseveranceData(userId) {
    ...
    try {
        ...
        if (docSnap.exists()) {
            ...
            if (perseveranceData.weekId !== weekId) {
                console.log(`[loadPerseveranceData] Week changed from ${perseveranceData.weekId} to ${weekId}. Clearing weekly interactions.`);
                perseveranceData.weeklyInteractions = {}; // Limpa interações da semana passada
                perseveranceData.weekId = weekId;         // Atualiza o ID da semana
                // Opcional: Salvar imediatamente a limpeza? Decidimos esperar a próxima interação.
            } else {
                 // Garante que weeklyInteractions exista como objeto
                perseveranceData.weeklyInteractions = perseveranceData.weeklyInteractions || {};
            }
