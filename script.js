...
// --- SELETOR PARA O NAVEGADOR FLUTUANTE ---
...
async function loadDataForUser(user) {
    try {
        ...
        // Verifica por alvos com prazo vencido após todos os dados serem carregados
        const now = new Date();
        const todayUTCStart = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()));
...
// --- LÓGICA DO NAVEGADOR FLUTUANTE ---

/**
 * Controla a visibilidade do navegador flutuante com base na posição de rolagem.
 */
function toggleFloatingNavVisibility() {
    if (!floatingNav || !state.user || state.prayerTargets.length === 0) {
        floatingNav?.classList.add('hidden');
        return;
    }

    const buffer = 50; // Distância do final da página para começar a esconder
...
function setupFloatingNavListeners() {
    ...
    floatingNav.addEventListener('click', (event) => {
        ...
        event.preventDefault(); // Previne o salto brusco da âncora
...
document.addEventListener('DOMContentLoaded', () => {
    onAuthStateChanged(auth, user => {
        ...
        if (user) { 
            loadDataForUser(user).finally(() => {
                toggleFloatingNavVisibility(); // Garante que a visibilidade seja checada após os dados carregarem
            });
        } else { 
            handleLogoutState();
            toggleFloatingNavVisibility(); // Esconde o nav ao deslogar
        }
    });
        ...
        switch(action) {
            ...
            case 'toggle-priority': {
                ...
                try {
                    ...
                    target.isPriority = originalStatus;
                    applyFiltersAndRender('mainPanel');
                    UI.renderPriorityTargets(state.prayerTargets, state.dailyTargets);
                }
                break;
    ...
    // --- INICIALIZAÇÃO DO NAVEGADOR FLUTUANTE ---
    setupFloatingNavListeners();
    toggleFloatingNavVisibility(); // Define estado inicial ao carregar
    window.addEventListener('scroll', toggleFloatingNavVisibility);
    window.addEventListener('resize', toggleFloatingNavVisibility);
});