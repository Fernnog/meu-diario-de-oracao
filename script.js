...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- DELEGAÇÃO DE EVENTOS PARA AÇÕES DINÂMICAS (INCLUINDO LÓGICA DE SUB-ALVOS E PRAZOS) ---
    document.body.addEventListener('click', async e => {
        ...
        // --- FUNÇÃO AUXILIAR PARA ENCONTRAR ALVO NO ESTADO LOCAL (MELHORIA ARQUITETÔNICA) ---
        ...
        switch(action) {
            ...
            case 'save-deadline': { 
                ...
                UI.toggleEditDeadlineForm(id, null); // Fecha o formulário
                applyFiltersAndRender(panelId);

                try {
                    await Service.updateTargetField(state.user.uid, id, isArchived, { hasDeadline: true, deadlineDate: newDeadlineDate }); 
                } catch(error) {
            ...
            case 'remove-deadline': {
                ...
                try {
                    await Service.updateTargetField(state.user.uid, id, isArchived, { hasDeadline: false, deadlineDate: null });
                } catch(error) {
                    alert("Falha ao remover prazo. A alteração será desfeita.");
                    target.deadlineDate = oldDeadlineDate;
                    target.hasDeadline = oldHasDeadline;
                    applyFiltersAndRender(panelId);
                }
