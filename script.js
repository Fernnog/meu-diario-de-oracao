...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Refatorado para usar UI Otimista
    document.body.addEventListener('click', async e => {
        ...
        // Funções auxiliares para UI Otimista
        const findTargetInState = (targetId) => {
            let target = state.prayerTargets.find(t => t.id === targetId);
            if (target) return { target, sourceList: 'prayerTargets' };
            target = state.archivedTargets.find(t => t.id === targetId);
            if (target) return { target, sourceList: 'archivedTargets' };
            target = state.resolvedTargets.find(t => t.id === targetId);
            if (target) return { target, sourceList: 'resolvedTargets' };
            return { target: null, sourceList: null };
        };
        ...
        switch(action) {
            ...
            case 'download-archived': {
                // ... (a lógica de download é síncrona, não precisa de otimização)
                const formatDateForDoc = (date) => date ? date.toLocaleDateString('pt-BR') : 'N/A';
                const sanitize = (text) => (text || '').replace(/</g, "&lt;").replace(/>/g, "&gt;");

                let htmlContent = `...`; // Lógica de geração do .doc aqui...
                const blob = new Blob([htmlContent], { type: 'application/msword' });
                const url = URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = `historico_${(target.title || 'alvo').replace(/[^a-z0-9]/gi, '_')}.doc`;
                a.click();
