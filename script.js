...
async function loadDataForUser(user) {
    try {
        ...
        // **MODIFICADO: Passa os dados diários para a renderização do painel de prioridades**
...
// **MODIFICADO: Função 'handlePray' com lógica de UI Otimista**
async function handlePray(targetId) {
    // Encontra o alvo nas listas de alvos ativos (de onde vêm os alvos diários e prioritários)
    const targetToPray = state.prayerTargets.find(t => t.id === targetId);
    if (!targetToPray) return;

    // Previne cliques múltiplos se a ação já foi registrada no estado local
    const isAlreadyCompleted = state.dailyTargets.completed.some(t => t.id === targetId);
    if (isAlreadyCompleted) return;

    // 1. ATUALIZAÇÃO OTIMISTA DA UI
    // Move o alvo da lista de pendentes para a de concluídos no estado local
    ...
    if (targetIndex > -1) {
        ...
        // Se não estava na lista de pendentes (ex: clicado do painel de prioridades), apenas adiciona aos concluídos
        state.dailyTargets.completed.push(targetToPray);
    }

    // Re-renderiza imediatamente os painéis afetados com o estado atualizado
    UI.renderDailyTargets(state.dailyTargets.pending, state.dailyTargets.completed);
    UI.renderPriorityTargets(state.prayerTargets, state.dailyTargets);

    // 2. AÇÕES DE BACKEND (em segundo plano)
    try {
        // Registra a oração e a interação no Firestore
        ...
        // 3. SINCRONIZAÇÃO DE DADOS PÓS-AÇÃO
        // Busca apenas os dados que mudam (perseverança) para atualizar a UI
        ...
        // Atualiza o estado local com os novos dados do backend
        state.perseveranceData = perseveranceData;
        state.weeklyPrayerData = weeklyData;

        // Atualiza a UI da perseverança com os dados frescos
        UI.updatePerseveranceUI(state.perseveranceData, isNewRecord);
        UI.updateWeeklyChart(state.weeklyPrayerData);

    } catch (error) {
        // 4. ROLLBACK em caso de erro no backend
        ...
        if (completedIndex > -1) {
            ...
            // Adiciona de volta aos pendentes apenas se ele estava lá originalmente
            if (state.dailyTargets.targetIds.includes(targetId)) {
                 state.dailyTargets.pending.unshift(revertedTarget); // Adiciona no início
            }
        }
        
        // Re-renderiza a UI para refletir o estado revertido
        UI.renderDailyTargets(state.dailyTargets.pending, state.dailyTargets.completed);
        UI.renderPriorityTargets(state.prayerTargets, state.dailyTargets);
    }
}
...
function handleGenerateCategoryView() {
    const allTargets = [...state.prayerTargets, ...state.archivedTargets]; UI.toggleCategoryModal(true, allTargets);
}
...
document.addEventListener('DOMContentLoaded', () => {
    onAuthStateChanged(auth, user => {
        ...
        if (user) { loadDataForUser(user); } else { handleLogoutState(); }
    });
    ...
    document.body.addEventListener('click', async e => {
        ...
        switch(action) {
            ...
            case 'pray-priority': await handlePray(id); break; // **NOVO: Ação para o botão de prioridade**
            case 'select-manual-target': try { await Service.addManualTargetToDailyList(state.user.uid, id); UI.toggleManualTargetModal(false); await loadDataForUser(state.user); alert("Alvo adicionado!"); } catch (error) { console.error(error); alert(error.message); } break;
            case 'resolve': if (confirm("Marcar como respondido?")) { if (target) { await Service.markAsResolved(state.user.uid, target); await loadDataForUser(state.user); } } break;
            case 'archive': if (confirm("Arquivar este alvo?")) { if (target) { await Service.archiveTarget(state.user.uid, target); await loadDataForUser(state.user); } } break;
            case 'delete-archived': if (confirm("EXCLUIR PERMANENTEMENTE?")) { if (target) { await Service.deleteArchivedTarget(state.user.uid, id); await loadDataForUser(state.user); } } break;
            case 'toggle-observation': UI.toggleAddObservationForm(id); break;
            case 'save-observation': { const text = document.getElementById(`observationText-${id}`).value.trim(); const dateStr = document.getElementById(`observationDate-${id}`).value; if (!text || !dateStr) return alert("Preencha o texto e a data."); const newObservation = { text, date: new Date(dateStr + 'T12:00:00Z') }; if (target) { await Service.addObservationToTarget(state.user.uid, id, !!target.archived, newObservation); await loadDataForUser(state.user); } break; }
            case 'edit-deadline': UI.toggleEditDeadlineForm(id, target?.deadlineDate); break;
            case 'save-deadline': { const newDeadlineStr = document.getElementById(`deadlineInput-${id}`).value; if (!newDeadlineStr) return alert("Selecione a nova data."); const newDeadlineDate = new Date(newDeadlineStr + 'T12:00:00Z'); await Service.updateTargetField(state.user.uid, id, !!target.archived, { deadlineDate: newDeadlineDate }); await loadDataForUser(state.user); break; }
            case 'edit-category': UI.toggleEditCategoryForm(id, target?.category); break;
            case 'save-category': { const newCategory = document.getElementById(`categorySelect-${id}`).value; await Service.updateTargetField(state.user.uid, id, !!target.archived, { category: newCategory }); await loadDataForUser(state.user); break; }
            
            case 'toggle-priority': {
                if (!target) return;
                
                const originalStatus = target.isPriority;
                const newStatus = !originalStatus;

                // 1. Atualiza o estado local PRIMEIRO para feedback instantâneo
                target.isPriority = newStatus;
                
                // 2. Re-renderiza APENAS os painéis afetados
                applyFiltersAndRender('mainPanel'); 
                UI.renderPriorityTargets(state.prayerTargets, state.dailyTargets);

                try {
                    // 3. Persiste a mudança no Firestore em segundo plano
                    await Service.updateTargetField(state.user.uid, id, !!target.archived, { isPriority: newStatus });
                } catch (error) {
                    // 4. Se a chamada falhar, reverte a mudança no estado e na UI
                    console.error("Falha ao atualizar prioridade no Firestore:", error);
                    alert("Ocorreu um erro ao salvar a prioridade. A alteração foi desfeita.");
                    target.isPriority = originalStatus; // Reverte o estado
                    applyFiltersAndRender('mainPanel'); // Re-renderiza para mostrar o estado original
                    UI.renderPriorityTargets(state.prayerTargets, state.dailyTargets);
                }
                break;
            }
